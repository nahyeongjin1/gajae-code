...
- `gjc notify bind-thread --session-id <id> --thread-ts <root>` adopts an existing Slack root for a live SDK session in the already configured workspace and channel, without posting a replacement root and without stopping the running daemon. The CLI is never a direct mapping writer: it proves the configured target and the exact current daemon owner, then submits the mutation over a new per-request chat-daemon command channel that the owner serves in place. Requests are addressed to an exact `ownerId`/`pid`/`incarnation`/daemon-generation tuple, and that whole tuple is re-proven both before dispatch and again inside the store commit fence, so an owner replaced or restarted after dispatch answers `owner_changed` and mutates nothing; the success answer echoes the same tuple and the CLI verifies it. The channel arbitrates the commit-versus-timeout race through a single-winner response object created with `O_CREAT|O_EXCL`: a caller that gives up must win that object before it reports failure, so a definitive timeout provably precludes any later mutation, and a caller that loses the race reports an unknown outcome instead of a failure. Every request/response file is opened, published, renamed, and removed under retained managed-filesystem authority — exact command-directory identity re-proven per syscall, owner-only permissions verified and repaired, no symlink traversal, and no non-regular entries — so a swapped or symlinked command path fails closed before the handler is reached. Adoption requires exact session authority (intact index replay, live non-terminal record, readable non-stale discovery endpoint owned by the indexed host pid at the same endpoint generation) plus daemon-side provider verification that the root really exists in the configured channel; the mapping commit re-proves session authority inside the store lock, so an authority change mid-flight leaves no mapping. Binding and stock root publication now serialize on one session root-claim invariant, so a bind racing a first notification publishes zero roots and leaves exactly one active adopted root. Operator-supplied timestamps are bounded ASCII `<seconds>.<fraction>` values, the command grammar rejects positional arguments and every unrelated notify flag at the real command entrypoint, and the success confirmation prints identifiers only.
- Adopting an existing Slack root is now a deterministic three-phase lifecycle: prepare session authority → bind the existing root through the live daemon → activate readiness. A session started with `GJC_NOTIFY_BIND_EXISTING_THREAD=1` is *prepared*: it publishes its endpoint and broker registration exactly as usual, so its id and endpoint generation are discoverable authority, but it withholds the replayable `session_ready` signal, which is what previously forced the daemon to publish a stock root before an operator could name an existing one. `gjc notify bind-thread` is unchanged and adopts the root through the running owner; the new `gjc notify activate-thread --session-id <id>` then asks the session's own host to publish the readiness it withheld. Activation is authorized against the daemon-owned mapping — an active record for exactly this session, in the configured workspace and channel, at exactly this endpoint generation — so activation before a binding fails closed (`not_bound`) with no grace period, and an unreadable mapping store is reported as unavailable authority rather than read as an authorization. Activation is exact-generation and idempotent: readiness is published once, a retry answers `already`, a request naming another session or a rolled generation is refused, and a lost answer is reported as indeterminate (a retry then settles it) instead of as a failure. A session that stops or rolls while its authorization is in flight publishes nothing. The deterministic ordering probe that previously ended in `session_conflict` now binds the exact operator root and produces zero replacement roots, including when the daemon attaches only after the binding and replays the retained readiness. Sessions without the explicit opt-in keep the stock immediate ready-root behaviour, and every existing global/process/session notification opt-out is unchanged. Preparation has exactly two authorities and they never overlap: the environment opt-in above for a manually started session, and the broker-issued `readiness` intent below for a lifecycle-managed one.
- Existing-thread preparation is now orchestrable through the broker and the Coordinator, not only through a manually started session's environment. `session.create` accepts a session-scoped `readiness` intent: `deferred` launches the child prepared, and the lifecycle wait completes on a distinct authenticated `session_prepared` signal instead of `session_ready`, so the create receipt reports `readiness: "prepared"` while no consumer has been told the session is live. The intent is admitted only as one of the two exact enum values and only for `session.create`; anything else is refused as `invalid_input`, an absent value keeps the stock immediate contract, and the intent travels on the launch request and the startup capability rather than any process-global environment, so an inherited flag can never defer a broker-created session (`GJC_NOTIFY_BIND_EXISTING_THREAD=1` is now explicitly refused for lifecycle-managed sessions). The session host gained the matching `session_activate` request — forwarded as a v3 frame by the Rust transport — which publishes the withheld readiness exactly once behind the same activation gate, re-proving startedness and generation after the asynchronous gate resolves and answering an exact retry `already`.
- `gjc_coordinator_start_session` accepts `prepare_existing_thread`, and the new `gjc_coordinator_activate_session` tool completes the sequence. A prepared start refuses an initial prompt before any broker mutation or idempotency record, records the durable session state as `prepared` (not ready for input), and is idempotent; a create that did not come back prepared is closed rather than reported as prepared. `gjc_coordinator_send_prompt` refuses a prepared session with `session_not_activated` until it is activated. Activation proves the same incarnation-bound workspace/generation authority the Coordinator already requires for endpoint use and then delegates to the shared activation exchange, so the Coordinator never writes the conversation store and never fabricates a readiness signal: durable state moves to `ready_for_input` only on a proven `activated`/`already`, a refused activation (`not_bound`, `not_prepared`, `session_not_live`, `activation_unavailable`) leaves the session prepared, and a lost answer stays indeterminate for an exact retry to settle. That shared exchange lives in a settings-free module so the Coordinator MCP entrypoint keeps its enforced module-closure boundary.
- Existing-thread preparation now fails closed instead of producing a prepared session that could activate with no binding, and the prepared signal no longer leaks into chat. The activation gate *is* the existing-thread bind authority, so the production bus derives preparation from it: a request that cannot build the gate — no configured, session-enabled Slack target, or no agent directory holding the daemon-owned mapping — settles a sanitized lifecycle startup failure (and throws for the `GJC_NOTIFY_BIND_EXISTING_THREAD=1` opt-in) rather than degrading to ordinary immediate readiness or handing back an unprotected prepared receipt. Previously a broker `readiness: "deferred"` create with no Slack target returned a prepared session whose endpoint answered `session_activate` with `activated` before any thread was bound, and the environment opt-in silently dropped the request. Separately, `ChatDaemonRuntime` now treats `session_prepared` as control-plane evidence only: it was previously rendered as ordinary content (`GJC event`) and posted to Slack, which claimed the very root the prepare phase exists to withhold — on replay, on live frames, and even for a stale endpoint generation. It now publishes no root, no thread message, no mapping, and no resume at any generation, while `session_ready` after a binding still adopts the exact operator root with zero replacement roots. Ordinary sessions without a preparation request keep the stock immediate-ready contract unchanged.
- Slack thread adoption now carries an explicit commit-certainty boundary end to end. `ConversationStore` reports a failed persistence attempt as either `ConversationCommitRefusedError` (the replacement rename never applied, so nothing changed) or `ConversationCommitUncertainError` (the rename applied and only the durability barrier after it failed), and an applied-but-unproven document is never deleted or rewritten. That certainty travels through the daemon handler as a typed `{ ok: false, certainty: "rejected" | "unknown" }` outcome, a new `outcome_unknown` response status, and the `binding_outcome_unknown` CLI code, so an applied mapping can no longer be reported as a definitive rejection such as `binding_failed` or `root_not_found`. A handler that throws after the fence granted commit authority is also reported as indeterminate, because an exception carries no evidence either way.
- The mapping commit now has an explicit logical linearization point: the instant the commit decision observes session authority. `transactWithSnapshot` accepts a `finalize` decision that runs *after* the replacement document is already published, not before it, because a proof taken before an asynchronous rename cannot cover the rename itself. The published document carries a `pending` marker that keeps it invisible to every reader, so authority that rolls during the publication is still observed: a decision that holds republishes the document without the marker, and a decision that fails rolls the previous mapping back and returns a definitive, mutation-free refusal. Only a rollback that cannot be proven complete is indeterminate (`binding_outcome_unknown`), so a refusal that reaches the caller always means nothing is applied. Session liveness, endpoint generation, and the exact daemon owner tuple are all re-proven at that point; a change after it is an ordinary lifecycle event handled by generation fencing. A provisional replacement left behind by a crashed writer is rolled back by the next lock holder, and readers never observe one. No provider or network call runs under the store lock.
- The chat-daemon command channel now runs on genuine retained filesystem authority instead of a pathname recheck. A new `cfg(unix)` native (`openRetainedCommandDir`/`RetainedCommandDir`) resolves the agent directory once as a trust root, walks the managed `sdk/daemons/<kind>/commands` suffix component by component without following links, repairs and re-proves owner-only permissions on the opened descriptor, and retains it; every list, stat, exclusive create, read, rename, link, unlink, and directory flush is then a `*at` syscall relative to that descriptor. Replacing, moving, or symlinking the command pathname after capture can no longer redirect a claim, read, publication, or removal outside the managed root, and reads still require a regular, single-linked, owner-only file. Hosts without descriptor-relative filesystem authority fail closed rather than downgrading to pathname operations.
- A settled request identifier now has durable replay semantics. Whoever wins the single-winner response claim records a bounded `<id>.settled.json` arbitration record — including a `committing` marker written inside the commit fence before the mutation — and submitter cleanup deliberately does not remove it. Resurrecting still-unexpired request material under a settled identifier no longer buys a second dispatch or mutation, a cancelled submission leaves replay authority behind, an exact idempotent retry discovers the prior terminal or indeterminate outcome without repeating provider proof or mutation, and an identifier reused with a different owner tuple, session, root, or command fails closed. Retirement of a retained record is genuinely identity-bound rather than name-bound: POSIX has no conditional `unlink`, so every create, link, rename, and removal in the command directory now serializes on a `flock` held on the retained directory descriptor itself — released by the kernel on close or process death, so there is no lock file to reclaim — and the removal then moves the proven object to a private single-use name, re-proves it there, and removes it through that name. A name replaced by anything outside the protocol is restored exactly where it was and reported as `identity_mismatch`, so a successor is never retired and its replay authority is never lost. Hosts that cannot serialize directory mutations fail closed at capture.
- Every daemon response is now validated against the complete request envelope before any status can affect caller behaviour. Answers echo the addressed request's `requestId`, `kind`, `command`, `ownerId`, `pid`, `incarnation`, daemon generation, session, and root, and a document that disagrees on any of them is treated as channel corruption (`untrusted`/`response_envelope_mismatch`) rather than as a success or a trusted business rejection — for `ok`, `rejected`, `owner_changed`, `expired`, and `outcome_unknown` alike. Output remains identifiers only.
- Directory durability is now proven rather than assumed. `writeScopedJson`, `publishScopedJsonExclusive`, and the native `syncDir` no longer discard a failed directory barrier: an applied namespace change whose durability cannot be proven is reported as a typed `durability_unknown`, and a host that refuses the barrier outright reports `durability_unsupported` instead of being silently downgraded to success. The command channel never converts that into a definitive `ok`, `rejected`, or `cancelled`. The `committing` replay marker must be *proven* durable before the commit fence grants authority, and a terminal settlement whose upgrade cannot be proven durable is answered `outcome_unknown` — matching the durable worst case, which is the indeterminate marker underneath it.
- A lock whose cleanup fails after the mapping is already committed no longer reports a definitive failure. `ConversationStore` releases its lock without rewriting the operation's own outcome and raises `ConversationLockCleanupError` carrying exactly what the completed operation returned, so lock-cleanup certainty is tracked separately from commit certainty: an applied mapping is reported as `binding_outcome_unknown`, a refused one keeps its definitive rejection, and a lock this process cannot prove it released is never deleted or reclaimed behind itself.
- The mapping commit's logical activation is now bracketed by two authority proofs instead of one. Publishing the replacement and proving authority cannot be a single act, so `ConversationStore` publishes the replacement `staged` (every reader still resolves the key to what it displaced), proves authority, publishes it `activating`, proves authority again, and only then publishes it `confirmed`. Authority here is monotone: a rolled endpoint generation, a replaced daemon owner tuple, a lost single-winner response claim, and a retired request never come back, so authority that is live on both sides was live across the whole activating publication. A proof that fails on any side rolls the previous mapping back and returns a definitive, mutation-free refusal; only a rollback that cannot be proven complete is `binding_outcome_unknown`. Reads take the same lock — but only when the document actually carries a marker — so no reader can observe the interval between a publication and the proof that decides it, and a marker left by a crashed writer is resolved by the next lock holder through the one rule every reader shares.
- A provisional publication is no longer assumed to survive an unflushed rename. Every publication inside the commit fence carries its own directory barrier, because POSIX does not promise that a rename whose parent directory was not flushed leaves the previous document behind. A publication whose barrier cannot be proven is rolled back rather than trusted: a proven rollback is a definitive refusal, an unproven one is `binding_outcome_unknown`. Malformed marker state — an empty key, a non-positive or non-integral owner pid, an empty incarnation, a non-finite timestamp, an unknown phase, or a displaced mapping without a usable generation — now fails the whole document closed instead of being handed back as an arbitrary record.
- Identity-bound retirement no longer sacrifices a third object to restore a second one. Repeated non-participating renames could install `B` at a name after its identity was proven and then install `C` after `B` was captured for retirement, and the replacing `renameat` used to put `B` back destroyed `C`. The restore is now an atomic no-replace rename (`renameatx_np(RENAME_EXCL)` on macOS, `renameat2(RENAME_NOREPLACE)` on Linux), and a host or filesystem without that primitive fails closed at capture rather than downgrading to a replacing rename. When the canonical name is occupied, the captured object is quarantined under a name that is deliberately not retirement residue: no sweep ever reclaims it, `list` never exposes it, and the call reports `identity_unrestored`. Every guarantee in this protocol — `flock`, directory `fsync`, and the no-replace rename — is claimed for local filesystems on a single host only; a network mount such as NFS or SMB can accept all three while providing weaker semantics.
- Every terminal settlement, not only a successful one, must be provably durable before the answer that depends on it is published. A `rejected`, `owner_changed`, or `expired` settlement whose directory barrier fails can be lost by a crash, which restores the still-unexpired request for another dispatch that may mutate — so the answer is downgraded to `outcome_unknown` with `settlement_durability_unknown` and the retained record is downgraded to match, keeping an exact replay consistent with what the caller was told. A settlement that is proven durable keeps its exact status.
- `CHAT_DAEMON_GENERATIONS` moves to 21 for both Discord and Slack, because the protected serving and answering contracts changed: an owner running an earlier generation may not serve or answer a request captured against this contract, and answers a mismatched request `owner_changed` without performing work.
- A durable activation marker is no longer read as a committed mapping. The `activating` document is published *before* the closing authority proof, so a writer that dies between the two leaves a replacement whose right to activate was never proven — and recovery used to keep it, restoring neither the displaced mapping nor an honest indeterminate outcome. `ConversationStore` now has a third durable phase: the closing proof's completion is itself written into the namespace as a `confirmed` marker carrying a `ConversationActivationProof` (the exact replacement generation, the clock reading at which the proof returned live, and the writer that took it), and that publication — not the activating one — is the linearization point at which the replacement becomes the mapping. `staged` and `activating` therefore both resolve to what the replacement displaced, so a crash anywhere before the closing proof is durably recorded rolls back to the previous mapping or to no mapping at all, and a restore that cannot be proven durable is `binding_outcome_unknown` rather than a stale success. Recovery never infers a proof that is not in the namespace. Confirmation evidence is validated against the document that carries it: missing evidence, evidence naming a different writer than the marker, evidence for a replacement the document does not carry, and evidence attached to a phase whose closing proof never ran all fail the whole document closed. One more authority proof runs after the confirming publication, so a roll that lands inside it is rolled back instead of reported as a success. The store lock's tenure is now proven rather than assumed on both sides of that publication, and a lock this process can no longer prove it holds is reported as indeterminate and never rolled back over or unlinked, so a stale-lock reclaim cannot hand the namespace to a second writer mid-transaction unnoticed. The document format change needs no daemon generation bump: an owner on an earlier generation fails a `confirmed` document closed rather than misreading it, and this contract rolls an earlier `activating` document back conservatively.
- Chat-daemon event handling now correlates one event identity before it acts, instead of taking the session id, the event name, and the generation from whichever representation happens to supply each. An event envelope and its payload are two representations of one event, not two authorities: a frame that names a different session in each, that carries a lifecycle marker (`session_prepared`, `session_ready`, `session_closed`, `session_terminated`) on one side under a different identity on the other, or that disagrees on a lifecycle generation is now dropped whole — on the replay path and the live path alike, with no close, resume, notification, action resolution, root, or mapping mutation. Previously the payload could supply the session id that cleared the attachment filter while the envelope supplied an ordinary event name, so a payload-smuggled `session_prepared` reached generic fanout and claimed the very root the prepare phase exists to withhold, and a foreign envelope's `session_ready` could resume through an attached payload session id. Ordinary wrappers are unaffected: an envelope may still name a transport-level event such as `notification` while its payload carries an unrelated event `type`, and canonical host lifecycle frames keep their exact behaviour, including control-plane-only `session_prepared` and exact-generation-fenced `session_ready`.
- `gjc_coordinator_activate_session` no longer seals `activation_outcome_unknown` as a completed idempotency replay. That outcome states only that the activation's result could not be observed — the session may already have published the readiness it was asked for — so caching it as terminal made the key answer that uncertainty forever and never sent the idempotent activation frame that would have answered `already`. The receipt now stays `in_progress` under its original request digest, so an exact same-key retry re-runs the same workspace-, generation-, and incarnation-proven activation and settles it: `already` moves durable state to `ready_for_input` with exactly one readiness publication and one start record, repeated unknown outcomes stay retryable until a settled answer closes the receipt, and a crash-left in-progress activation is recoverable under the same key. Deterministic outcomes such as `not_bound` remain terminal replays with no second activation attempt, a reused key naming a different session still fails `idempotency_conflict`, concurrent same-key retries stay serialized on the receipt lock, and every other Coordinator tool keeps its existing terminal-caching behaviour unchanged.

### Fixed

- Slack Web API requests now use form encoding instead of JSON, so posting, thread reconciliation, and existing-root verification through `conversations.replies` no longer fail with `invalid_arguments`.
- The common chat-daemon runtime no longer publishes SDK protocol traffic as chat content. `SdkClient` settles a request's pending promise and still forwards that same frame to every `onFrame` observer, so `ChatDaemonRuntime` received the `event_replay_result` answer to the `event_replay` it issues on every attachment — and projected it through the generic unknown-frame path, rendering `GJC event replay result` and publishing it as a brand-new Slack root wherever no mapping existed. An explicit, exact, typed classifier (`CONTROL_PLANE_FRAME_TYPES`) now drops `event_replay_result`, `control_response`, `query_response`, and the handshake `hello` at the runtime seam, ahead of presentation fanout and of every root, mapping, resume, close, and action mutation, on the live path and the replay path alike; membership is exact rather than a `*_result` suffix rule, so ordinary public frames are untouched. A control-plane discriminant is also a reserved identity for envelope correlation, so a protocol response smuggled under an ordinary `notification` envelope fails closed as a whole frame and its nested `events` are never reprojected. `session_prepared` stays inert, `session_ready` keeps its exact-generation readiness, and Telegram's existing local guard is unchanged as defence in depth.
- SDK host and broker test fixtures no longer reach a real agent directory. `createNotificationsExtension` resolves the process-global `Settings` singleton whenever a caller supplies none, and `Settings.isolated()` — which takes no agent-directory option — falls back to the process-global `getAgentDir()` even when the fixture passed an explicit `agentDir` alongside it. `startSession` then spawns a broker, opens a `SessionIndex`, and appends `host_registered` under whatever directory that resolution reports, so in a multi-file `bun test` cohort one unrelated `Settings.init()` made `sdk-host-wiring` fixtures register into a developer's live index and read its live chat destinations, and `sdk-workflow-gate-emitter` did the same through `Settings.isolated()` alone. Every fixture now supplies an explicit, notification-free authority bound to its own temp agent directory with no ambient `getAgentDir()` fallback, and two executable regressions prove it: a canary singleton that must be neither read nor written while a fixture owning a temp agent directory still registers there, and an assertion that the ambient session index gained no row locating any root the suite owned. The `sdk-broker-lifecycle-e2e`, `sdk-daemon-cli-e2e`, `sdk-broker-source-isolation`, `sdk-default-model-selection-e2e`, `sdk-workflow-gate-emitter`, and `notifications-lifecycle-create-autoresume` suites now settle every child, broker, session host, listener, and process-global they own in `finally` and assert their own orphan count is zero. A detached broker is settled through the owned handle where one exists and otherwise through its own temp agent directory's discovery record — never a process name, command line, or pid alone: the recorded OS process-start incarnation must still be the one that pid is running, re-proven immediately before each signal and throughout each grace period, so a stale record can never signal a reused pid. Because most of these tests delete their temp root in their own `finally`, that identity is captured while the discovery record still exists rather than read back after it is gone.
- Chat-daemon envelope correlation now treats a duplicated identity as one authority tuple for every event class, not only for lifecycle signals. A frame whose envelope and payload both state a `generation` or a `sessionId` must state the same well-typed value in each, so an ordinary `notification` wrapper carrying a conflicting generation — or a duplicate that cannot be that identity at all, such as a non-integral, negative, or non-numeric generation, or an empty or non-string session id — is dropped whole on the replay path and the live path alike, with no post, root, mapping, resume, close, or action mutation. A representation *owns* an identity by having the property at all, so a stated `undefined` is a malformed duplicate rather than an absent one on either side; previously the generation check ran only when the correlated name was a lifecycle event, and any unreadable duplicate was read as absence, which silently promoted the other representation to sole authority over a frame that stated two. One frame's event name is likewise one identity spelled by `name` and `kind` together: owning both obliges them to be well-typed and exactly equal, so a benign `name: "notification"` can no longer hide a reserved `kind: "control_response"`, `"session_closed"`, or `"event_replay_result"` — nor the reverse — and the frame is inert before its payload is projected at all. Single-sided identities are unchanged: an envelope that carries the identity or the name while its payload does not is still read from the side that supplied it, reserved lifecycle and control-plane identities keep their stricter name agreement, and legitimate marker and notification payloads are not over-filtered.
- A lifecycle rollback whose child had already exited is no longer recorded as durable cleanup uncertainty. `terminateSpawnedChild` observed the exact spawned process once at three points and then declared uncertainty from that single reading, but a child that is exiting still answers `kill(pid, 0)` while its OS start incarnation has already become unreadable, so a bounded exit race produced `uncertain` rather than `exited`. That was the whole window in which the failure occurred: a child that publishes an owned startup-failure receipt, releases its own endpoint and broker registration, and then dies is observed by the broker microseconds later, and the broker correctly refuses to signal a process it can no longer prove durably identified. Each of those readings now converges on the remaining lifecycle deadline through the existing timing seam instead of a blind sleep, so a `session.create` with no bind authority deterministically returns `spawn_failed` with a proven rollback, `session.list` stays empty, and no endpoint, marker, failure artifact, index row, or process survives. Fail-closed behaviour is unchanged: an exit that never becomes provable still exhausts the deadline, still returns typed `terminal_uncertain`, still retains its artifacts for reconciliation, and never authorizes signalling a reused or replacement pid.
- A durably persisted `terminal_uncertain` outcome now reads back as itself. `LifecycleLedger.readTerminal` accepted only `terminal_ok`/`terminal_error`, so the broker's own post-write verification could never verify an uncertainty it had just written: it reported a generic "terminal evidence could not be verified after persistence" in place of the proven reason, and appended a second final row for the same identity. That row is an invalid history continuation, so the next reopen quarantined it into `lifecycle-ledger.jsonl.corrupt` — a corruption produced entirely by the broker's normal verification path. Read-back now accepts any proven final state, and the unverified branch no longer appends a second final transition at all, because the transition preceding it already recorded this identity's final row and therefore already owns reconciliation. Uncertainty is never reclassified as success, and an outcome whose persistence genuinely cannot be verified is still reported as `terminal_uncertain`.
- A terminal outcome the broker could not read back is no longer promoted by the next same-key retry. Withholding the second final row left the durable ledger correct but the in-memory row untouched, so the retry took the remembered `terminal_ok`/`terminal_error` straight out of `#byIdentity` and returned the very response whose persistence the first caller had just been told was unverifiable — reconciliation-owned uncertainty silently became a success without any reconciliation. Every same-key replay of a terminal response now re-reads the durable terminal evidence first and returns the response only when the file still reproduces that exact row's identity, request hash, response digest and body, durable effects, and startup-failure receipt; while it cannot, the caller gets the same generic `terminal_uncertain` with no second ledger final and no durable-history mutation. A later retry whose authoritative read succeeds is explicit revalidation and may replay the original outcome. A different request hash is still fenced as `idempotency_conflict` ahead of replay, `cleanup_pending` reconciliation keeps its own re-execution semantics rather than a terminal-only rule, and a validly persisted `terminal_uncertain` still replays its own proven uncertainty instead of a generic one.
- The `session.delete` locator-reconstruction shorthand no longer bypasses that fence. A delete carrying only `{sessionId}` looks its identity up before the request hash is computed and before `begin()` runs, and it used to return that entry's stored response directly — so a key anchored by a full-locator delete could be retried as `{sessionId}` and receive the earlier outcome with no request-hash comparison and no durable re-read, promoting exactly the unverified success the rule above exists to withhold. No terminal outcome is returned from that pre-normalization branch any more: an entry that already reached a final state falls through to the normal path, where the minimal request is hashed as itself (so a full-locator key retried as `{sessionId}` is now `idempotency_conflict`) and a matching minimal replay is served only after `readTerminal` reproduces its row. The idempotent no-op for a session that is neither owned under this identity nor indexed is likewise recorded through that path instead of being answered inline, so it is fenced and verified like every other terminal outcome. Ledger evidence without replayable authority, `cleanup_pending` shorthand reconciliation, and the cross-identity pending-cleanup and uncertain-cleanup fences all keep their existing behaviour.
- Uncertainty recovered from a quarantined history no longer carries the prior row's success. A malformed, digest-invalid, request-hash-conflicting, or second-final history was recovered by copying the last valid entry and changing only its state, so a ledger holding a valid `terminal_ok` followed by a second final produced a `terminal_uncertain` that still contained `{ok: true}` and its response digest — and the broker replayed that success to the caller even though the history it came from was unexplainable. The synthesized uncertainty now preserves only the identity, its request hash, and explicit fail-closed cleanup fencing; the response and its digest are dropped, so neither `begin()` nor `readTerminal()` can hand back an outcome the ledger cannot prove. Recovery is also stable: no synthetic final is appended behind a history that already ends in a durable final row, so repeated reopens re-derive the same fence from the same quarantined evidence instead of growing a final-after-final chain, and a later compaction rewrites the identity as an accepted anchor plus that response-free uncertainty rather than normalizing the discarded success back into a replayable row. The offending row is still quarantined into `lifecycle-ledger.jsonl.corrupt`, and ordinary `accepted → terminal_ok`/`terminal_error`/`terminal_uncertain` histories and valid `terminal_uncertain` read-back are unchanged.
- A final row must now agree with the response it carries. Recovery validated digests but never the meaning of the state, so `accepted → terminal_uncertain(response={ok: true})` with a matching digest was accepted as ordinary history: `begin()` returned that uncertainty and the broker handed the stored success straight back, and `terminal_error` carrying `{ok: true}` replayed the same way once its durable read-back matched. Validation is now the exact inverse of `lifecycleResponseState`, the only mapping the broker ever writes — `terminal_ok` requires `ok === true`, `terminal_error` requires a definitive failure that is neither `terminal_uncertain` nor retained cleanup authority, and `terminal_uncertain` requires `ok === false` and never a success — so a contradictory row is quarantined and replaced by response-free, digest-free uncertainty that neither `begin()` nor `readTerminal()` can replay. `effect_started` cleanup-pending rows and unresolved cleanup fences keep their existing semantics, and a row whose response carries no `ok` discriminant is still treated as an opaque payload rather than a lifecycle outcome.
- A line that cannot be decoded at all now fails closed the identities it could belong to, including one that already reached a durable final. Recovery marked only non-final identities uncertain, so a valid `accepted → terminal_ok({ok: true})` followed by a complete but parse- or schema-impossible line left that success live in memory: `begin()` replayed it immediately, and a later size- or row-triggered compaction snapshotted the success and dropped the offending line from the source, normalizing the corruption away while keeping the outcome it made unexplainable. Unattributable corruption now derives sanitized memory-only uncertainty for every already-known identity — no response, no digest, and no row appended behind the durable final, so repeated reopens re-derive the same fence and compaction rewrites the identity as an accepted anchor plus that uncertainty instead of the discarded success. `readTerminal` already refuses a source it cannot decode, so the durable read-back stays unproven too, and the offending bytes are still retained in `lifecycle-ledger.jsonl.corrupt`.
- Quarantine no longer grows without bound on repeated reopens. Recovery is re-derived from the same durable source every time the ledger is opened, so permanent corruption was presented — and appended to `lifecycle-ledger.jsonl.corrupt` — again on every start. Re-retaining evidence that is already held byte-identically is now a no-op, so the sidecar's bytes, size, and digest are stable across reopens, while distinct corrupt evidence is still appended. The comparison reads the sidecar under the same owner-only, regular-file, no-follow, bounded contract the writer uses, so a swapped or symlinked sidecar still fails closed without its target being read or modified.
- A throwing spawn observer can no longer strand the exact child it was handed. The test-only seam ran immediately after `spawn()` — before the child's pid and OS incarnation were captured and before its durable lifecycle marker was written — so a throw left the failure path with no `spawnedAuthority` and no marker, and `terminateSpawnedChild` fell through to marker-based verified signalling that could not authorize anything: the request settled `terminal_uncertain` after its deadline while the exact child stayed alive with its lifecycle artifacts retained. The seam now runs only after that authority is captured and the marker is durable, still receiving the same retained `ChildProcess` from the original spawn, so a throw fails into a catch that signals and reaps exactly that process through the existing verified contract and returns a proven `spawn_failed` rollback with no live child and no endpoint, marker, failure-artifact, index, or ledger residue. An absent observer remains a no-op, and no new pid-, name-, group-, or tree-based signal authority was introduced.
